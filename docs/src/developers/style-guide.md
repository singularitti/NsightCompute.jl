...
!!! info
    The style guide is always a work in progress, and not all NsightCompute code
    follows the rules. When modifying NsightCompute, please fix the style violations
...
NsightCompute uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/NsightCompute.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the NsightCompute directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to NsightCompute have
